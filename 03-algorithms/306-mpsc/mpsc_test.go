...
var gMailchan = make(chan bool, 1)
var gBmailin [8]int64
...
func benchmarkPushPopActor(count, c int) {
	...
	go func() {
		...
		for {
			select {
			case <-gMailchan:
				atomic.StoreInt64(&gBmailin[0], 0)
	...
	for i := 0; i < c; i++ {
		go func(n int) {
			for n > 0 {
				q.Push(val)
				if atomic.LoadInt64(&gBmailin[0]) == 0 && atomic.CompareAndSwapInt64(&gBmailin[0], 0, 1) {
					gMailchan <- true
				}
				n--
			}
